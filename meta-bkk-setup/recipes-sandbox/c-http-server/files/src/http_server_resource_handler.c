...
#define HTTP_SERVER_STATIC_ROOT "./www"
...
static void send_simple_response(int client_fd, const char *status, const char *content_type,
                                 const char *body)
{
  char header[512] = { 0 };
...
void http_server_handle_resource_request(int client_fd, const char *request_text)
{
  ...
  if (sscanf(request_text, "%7s %255s %15s", method, request_path, http_version) != 3) {
    send_simple_response(client_fd, "400 Bad Request", "text/plain; charset=utf-8", "Bad Request\n");
    return;
  }

  if (strcmp(method, "GET") != 0) {
    send_simple_response(client_fd, "405 Method Not Allowed", "text/plain; charset=utf-8",
                         "Only GET is supported\n");
    return;
  }

  if (!is_safe_request_path(request_path)) {
    send_simple_response(client_fd, "403 Forbidden", "text/plain; charset=utf-8", "Forbidden\n");
    return;
  }
  ...
  if (copy_len >= sizeof(path_without_query)) {
    send_simple_response(client_fd, "414 URI Too Long", "text/plain; charset=utf-8", "URI Too Long\n");
    return;
  }
  ...
  if (snprintf(full_path, sizeof(full_path), "%s/%s", HTTP_SERVER_STATIC_ROOT,
  ...
  if (mime_type == NULL) {
    send_simple_response(client_fd, "415 Unsupported Media Type", "text/plain; charset=utf-8",
  ...
  if (stat(full_path, &file_stat) != 0 || !S_ISREG(file_stat.st_mode)) {
    send_simple_response(client_fd, "404 Not Found", "text/plain; charset=utf-8", "Not Found\n");
    return;
  }

  FILE *file = fopen(full_path, "rb");
  if (file == NULL) {
    send_simple_response(client_fd, "500 Internal Server Error", "text/plain; charset=utf-8",
