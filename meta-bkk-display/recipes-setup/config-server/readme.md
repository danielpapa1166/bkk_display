...
`config-server` is the C HTTP service that guides a device through initial
configuration. It serves the setup UI on port 8080, configures WiFi and the
BKK API details, and stops being launched once setup is complete.
...
The `config-server` executable is split into an entry point, GET and POST
handlers, user-action processing, and WiFi validation. It uses `chttp` for the
HTTP server, cJSON for request bodies, the BKK TEE client for secure values,
the common D-Bus utilities, and rbuflogd for logging. Static web assets are
installed separately from the executable.

## Integration

`config-server_0.1.bb` builds `/usr/bin/config-server` with CMake and installs
the UI assets under `/usr/share/config-server/www`. It depends on rbuflogd,
cJSON, chttp, BKK TEE, common utilities, and the project network manager; its
runtime package additionally requires wpa-supplicant and bash. Boot-mode
services decide whether to invoke it in WiFi or API mode, while
application-manager controls the broader device lifecycle. It does not install
or enable a dedicated service unit itself.
